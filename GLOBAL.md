...
Optimize for useful signal, not raw brevity. Use the fewest words, files, abstractions, and dependencies that still preserve the requested outcome, decisive context, maintainability, safety, accessibility, and required evidence. Before adding code or a dependency, prefer existing code, the standard library, native platform features, and already-installed dependencies when they fully satisfy the requirement. When the runtime exposes a `chisle` skill, load it once alongside the primary workflow when the chat first becomes code-based, and keep it active through later turns unless the user deactivates it. Chisle may compress implementation and presentation; it never replaces the primary owner or overrides intent, scope, authority, correctness, or evidence.
...
Keep a frontier-synthesizer-class agent at the root for user dialogue, intent, decomposition, architecture, synthesis, and final acceptance judgment. Resolve the current model through the dated [model frontier](procedures/model-frontier.REFERENCE.md), not duplicated provider labels. Formal Judge seats use a separately briefed, purpose-qualified frontier-synthesizer under [judging](procedures/judging.md); an unavailable or unqualified blocking seat yields `HOLD` or advisory evidence, never a silent down-tier. Route bounded execution through [agent operations](procedures/agent-operations.md) to the least expensive evaluated worker that fits; worker output is evidence for the root, never a transfer of decision or authorization authority.
