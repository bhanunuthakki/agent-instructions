...
| 15 | CODE READY; OWNER AUTH REQUIRED | Date Suggester commits `fc19804` and `1ed0cc1` add a crash-safe locked Mac runner, durable powered-off/login catch-up, same-slot duplicate suppression, and a Sunday 09:00 LaunchAgent template. All 430 tests pass; the plist and shell validate. The Windows job remains the sole active scheduler and the Mac agent remains unloaded because this Mac has neither `credentials.json` nor `token.json`. After fresh official Google OAuth: run a no-email canary, confirm Windows completed the current slot, seed that slot marker, disable and verify Windows, load Mac, then verify no duplicate digest. The Windows-only URI handler remains retired unless separately requested. |
