...
`user_enablement_cases.jsonl` is a separate eight-case response suite for the collaboration consolidation. It covers routine technical ownership, a consequential cloud/privacy choice, recognizable design exploration, evidence-backed challenge, inspectable delivery, an unexpected irreversible dependency, requested expert detail, and private-data protection in a shareable preview. It leaves the existing 32-case corpus and qualification history unchanged. Candidate prompts load the actual global, catalog, and named workflow sources; judge-only criteria remain hidden.
