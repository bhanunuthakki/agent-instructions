...
| Shared agent instructions | Portable source rules, hooks, bootstrap, and managed Codex CLI resolution are committed and pushed; 44 instruction tests and the artifacts-only drift check pass | Good locally; Mac test pending | Bootstrap all three agents and run both CLI transport checks on the Mac |
