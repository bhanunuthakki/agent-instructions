...
| 8 | DONE / INTENTIONAL SPLIT | Mac holds the development checkouts and recreates project environments from declared dependencies as needed. Earnings and Portfolio production execution/data intentionally remain Windows-only; their Windows environment, token directory, database, and scheduled jobs were not duplicated on the Mac. Resume's actively used Python environment was rebuilt, but Resume source reconciliation remains the separate owner hold in item 5. |
...
| Date Suggester | Mac `main` is aligned at `17dc68e`; four canary-refreshed personal learning-state files remain local | Keep those private files in the verified snapshot unless the owner explicitly approves publishing them | Mac-owned weekly job; Windows schedule and URI handler retired |
...
- [ ] The newest verified database, scratch, Claude-project, and Codex-memory backups exist locally on Mac and in Google Drive. Encrypted Time Machine is an accepted owner deferral until an external disk is purchased.
...
- [ ] FileVault is on. Encrypted Time Machine is an accepted owner deferral; the weekly verified Drive snapshot is the interim control.
