...
def relative_map(root: Path, artifacts: dict[Path, str]) -> dict[str, str]:
    return {path.relative_to(root).as_posix(): content for path, content in artifacts.items()}
...
def test_hardening_package_has_exact_nineteen_rubrics_and_runtime_closure(tmp_path: Path) -> None:
...
def test_all_four_runtime_layouts_are_byte_identical_by_relative_path(tmp_path: Path) -> None:
    roots = [
    ...
    packages = [relative_map(root, sync.build_harden_package_artifacts(root)) for root in roots]
