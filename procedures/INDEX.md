...
The primary owner carries the requested outcome and authorized actions. Additional owners contribute their boundary constraints and acceptance evidence to that same contract; do not repeat discovery questions or handoffs. Load a reference or specialist only when an actual decision or risk needs it. A qualified existing LLM route does not require a fresh model comparison; a small fix does not require product discovery; a new persisted identity does require data semantics. A documented project restriction remains binding until changed through its named authority.
